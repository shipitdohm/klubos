...
### 2026-07-20 — VS-0.8.2 Unicode-/Compound-Priorisierung nach VS-0.7.6-Gate

- Commit/Diff: `2e6e381` auf `origin/main`; die Suchvarianten priorisieren nun die belegten Unicode-/Compound-Formen vor verkürzten bzw. ungetrennten Varianten. Der konkrete API-Cache-Key bewahrt weiterhin unterschiedliche Rohschreibweisen getrennt. Produktmarker: `VS-0.8.2-official-alias-safe`.
- Specialist-Status: `READY_FOR_REVIEW` — lokale Tests, Push und öffentliche Zielabfragen abgeschlossen; unabhängiger 32+100-Gate steht aus.
- Reviewer-Baseline: Der unabhängige VS-0.7.6-Gate dokumentierte als P1 die ASCII-Zündorf-Varianten und die kombinierten Halle-Varianten; keine Produktänderung durch den Reviewer.
- Observed, öffentliche API am 20.07.2026 nach Push `2e6e381`: `TC Blau-Weiss Zundorf`, `Tennisclub Blau Weiss Zundorf` → jeweils HTTP 200, `unique_official_match`, genau 1 Treffer `TC Blau-Weiss Zündorf`, `nuliga:DTB:35409`, `sourceName: DTB nuLiga (offizieller Verbands-Ergebnisdienst)`, `confidence: high`, `logoState: fallback`. `TC Blau Weiss Halle`, `Tennisclub Blau-Weiss Halle e.V.` und `Tennisclub Blau-Weiss Halle Halle` → jeweils HTTP 200, `unique_official_match`, genau 1 Treffer `TC Blau-Weiß Halle`, `nuliga:DTB:26504`, mit denselben Quellen-/Confidence-/Logo-Feldern. Die Antwortfelder enthalten `sourceUrl` und `checkedAt`; die Abrufzeitpunkte liegen zwischen 15:15:30 und 15:16:39 UTC.
- Observed, öffentliche UI am 20.07.2026: `https://klubos.de/vereinssuche.html` liefert HTTP 200, `data-version="VS-0.8.2-official-alias-safe"` und `Live-Quelle VS-0.8.2`.
- Claimed: Die bekannten Zündorf-/Halle-Aliase werden vor dem DTB-Kandidatenvergleich auf den jeweils belegten offiziellen Vereinsdatensatz geführt; eine mehrdeutige oder fremde OSM-Entität wird dabei nicht als Ersatz ausgegeben.
- Inferred: Die beiden im VS-0.7.6-Gate offenen P1-Aliasfamilien sind in den gezielten öffentlichen Fällen behoben. Das ist noch kein Beleg für die vollständige 32er-/100er-Matrix.
- Hypothesis / offene Annahme: Die symmetrische Normalisierung deckt die im Vertrag festgehaltenen Kombinationen aus ASCII-Umlaut, Bindestrichverlust, `TC`/`Tennisclub`, `e.V.` und doppeltem Ortssegment ab; weitere unbekannte Namensvarianten müssen der unabhängige Live-Gate und die versionierte Alias-Matrix prüfen.
- Lokale Tests: `node scripts/vereinssuche-alias-contract.mjs` erfolgreich mit 18 Ankern und 113 Varianten; `node --check api/vereinssuche.js` und `node --check api/vereinssuche-normalization.js` erfolgreich; `git diff --check` erfolgreich; direkter Astro-Produktionsbuild erfolgreich (`7 page(s) built`, `Complete!`). `pnpm exec astro build` war im nicht-interaktiven Runner wegen der pnpm-Modulbereinigung abgebrochen; der direkte Build mit der vorhandenen Installation ist der erfolgreiche Build-Nachweis.
- Nächster Schritt: Search Reviewer prüft exakt `2e6e381` öffentlich mit Browser-/Quellen-/Regressionstests, der vollständigen 32er-Matrix und mindestens 100 sequenziellen Aliasvarianten. PASS/FAIL wird unabhängig im nächsten Log-Eintrag dokumentiert; bis dahin keine PASS-Behauptung.
