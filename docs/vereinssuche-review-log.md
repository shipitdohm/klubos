...
### 2026-07-20 — VS-0.7.2 kanonischer DTB-Aliasvertrag und sicherer OSM-Fallback

- Commit/Diff: `a7948fd` und `64a9d59` plus Follow-up im Working tree; `api/vereinssuche-normalization.js` führt die kanonische Namensnormalisierung, Aliasvarianten und stabile Identitätsschlüssel ein. `api/vereinssuche.js` fragt nuLiga mit TC/Tennisclub-, Unicode/ASCII-, Bindestrich- und e.V.-Varianten ab, beendet offizielle Mehrdeutigkeit ohne OSM-Ersatz und akzeptiert OSM nur bei kanonisch passendem Namen. Ein offizieller Namenszusatz wie `TSC Hansa Dortmund` wird als offizieller Treffer akzeptiert; generische Stadtanfragen wie `Tennisclub Köln` werden bei einem nur erweiterten Kandidatennamen als `ambiguous` behandelt. Die strenge Gleichheitsprüfung bleibt ausschließlich beim OSM-Fallback. `vereinssuche.html` zeigt VS-0.7.2 und erklärt den sicheren Fallback. `scripts/vereinssuche-alias-contract.mjs` plus `package.json` ergänzen den lokalen Vertragstest.
- Specialist-Status: `READY_FOR_REVIEW` nach Push und öffentlicher Verifikation.
...
- Inferred: Die gemeldeten Verwechslungen werden dadurch entweder auf den offiziellen DTB-Treffer normalisiert (`Tennisclub Großhesselohe`, `TC Grosshesselohe`, `Tennisclub Kirchhörde`, `Tennisclub Weiden`, `Tennisclub Bamberg`) oder sicher als `ambiguous`/`not_found` behandelt (`TC GH`, `Tennisclub Neuss`, `Tennisclub Köln`), statt einen ähnlichen Nachbarverein auszugeben. Die VS-0.7.2-Regressionsprobe bestätigt zusätzlich `TSC Hansa` → `TSC Hansa Dortmund` als offiziellen Treffer.
