...
### 2026-07-20 — VS-0.7.3 kanonischer DTB-Aliasvertrag und sicherer OSM-Fallback

- Commit/Diff: `a7948fd` → `64a9d59` → `26e6164` plus Follow-up im Working tree; `api/vereinssuche-normalization.js` führt die kanonische Namensnormalisierung, Aliasvarianten und stabile Identitätsschlüssel ein. `api/vereinssuche.js` fragt nuLiga mit TC/Tennisclub-, Unicode/ASCII-, Bindestrich- und e.V.-Varianten ab, beendet offizielle Mehrdeutigkeit ohne OSM-Ersatz und akzeptiert OSM nur bei kanonisch passendem Namen. Ein offizieller Namenszusatz wie `TSC Hansa Dortmund` oder `TC Augsburg Siebentisch` wird bei direkter nuLiga-Antwort akzeptiert; generische Stadtanfragen wie `Tennisclub Köln` werden bei einem nur erweiterten Alias-Kandidaten als `ambiguous` behandelt. Unbelegte kurze Kürzel wie `TCK` dürfen keinen OSM-Treffer erzeugen; bei `source_unavailable` gibt es keinen OSM-Ersatz. Die strenge Gleichheitsprüfung bleibt ausschließlich beim OSM-Fallback. `vereinssuche.html` zeigt VS-0.7.3 und erklärt den sicheren Fallback. `scripts/vereinssuche-alias-contract.mjs` plus `package.json` ergänzen den lokalen Vertragstest.
- Specialist-Status: `READY_FOR_REVIEW` nach Push und öffentlicher Verifikation.
...
- Inferred: Die gemeldeten Verwechslungen werden dadurch entweder auf den offiziellen DTB-Treffer normalisiert (`Tennisclub Großhesselohe`, `TC Grosshesselohe`, `Tennisclub Kirchhörde`, `Tennisclub Weiden`, `Tennisclub Bamberg`, `TC Augsburg`) oder sicher als `ambiguous`/`not_found` behandelt (`TC GH`, `TCK`, `Tennisclub Neuss`, `Tennisclub Köln`), statt einen ähnlichen Nachbarverein auszugeben. Die VS-0.7.3-Regressionsprobe bestätigt zusätzlich `TSC Hansa` → `TSC Hansa Dortmund` und `TC Augsburg` → `TC Augsburg Siebentisch` als offizielle Treffer.
...
- Lokale Tests: `node --check` für beide API-Module erfolgreich; `node scripts/vereinssuche-alias-contract.mjs` erfolgreich mit 18 realen Vereinsankern und 112 Varianten; lokaler Server-Adapter gegen echte nuLiga-/OSM-Quellen: `Tennisclub Großhesselohe` → 1 offizieller DTB-Treffer, `TC Grosshesselohe` → 1 offizieller DTB-Treffer, `TC GH` → 0 `not_found`, `Tennisclub Kirchhörde` → 1 offizieller DTB-Treffer, `Tennisclub Weiden` → 1 offizieller DTB-Treffer, `Tennisclub Bamberg` → 1 offizieller DTB-Treffer, `Tennisclub Neuss` → 0 `ambiguous`, `Tennisclub Köln` → 0 `ambiguous`, `TC Bredeney` → 1 offizieller DTB-Treffer, `TSC Hansa` → 1 offizieller DTB-Treffer; direkter Astro-Build erfolgreich (`7 page(s) built`, `Complete!`); `git diff --check` erfolgreich.
...
- Öffentliche VS-0.7.3-Verifikation nach Push `de6d242`: Seite liefert `data-version="VS-0.7.3-official-alias-safe"` und `Live-Quelle VS-0.7.3`. Die priorisierten Einzeltests liefern `TC Großhesselohe`, `Tennisclub Großhesselohe`, `TC Grosshesselohe`, `TC Kirchhörde`, `Tennisclub Kirchhörde`, `TC Weiden`, `Tennisclub Weiden`, `TC Augsburg` → `TC Augsburg Siebentisch`, `TSC Hansa` → `TSC Hansa Dortmund`, `TC Bamberg`/`Tennisclub Bamberg` und `TC Blau-Weiss Halle` jeweils als offiziellen DTB-/nuLiga-Treffer. `TC GH` und `TCK` liefern 0 ohne OSM-Ergebnis; `Tennisclub Köln`, `Tennisclub Neuss`, `Tennisclub Augsburg`, `Tennisclub Hansa` und `Tennisclub Iphitos` liefern 0 `ambiguous`/`not_found`; `zzzzzz tennisclub` liefert 0 `not_found`. Vollnamen mit kanonisch passendem OSM-Datensatz (`Tennisclub Grün-Gold Köln`, `Tennisclub Blau Weiß Zündorf`) bleiben als OSM gekennzeichnet und werden nicht als DTB ausgegeben. Browser: VS-0.7.3, Hansa-Detailansicht, DTB-nuLiga-Quelle, 0 `.vs-result-option`; Empty-Submit zeigt beide Hinweise mit `aria-invalid="true"`; Sentinel zeigt `Live-Suche momentan nicht verfügbar`. Öffentlicher Sentinel-API-Check: HTTP 502 mit `error: source_unavailable`, `matchState: source_unavailable`, `reviewSentinel: true`.
- 32er-Stressmatrix: Eine parallele öffentliche Probe lieferte 31 JSON-Antworten (`18 official`, `3 osm`, `6 ambiguous`, `4 not_found`) und einen reinen Socket-Reset; die betroffenen Einzelanfragen `TC Grosshesselohe`, `Tennisclub Weiden` und `TSC Hansa` wurden danach einzeln erfolgreich als offizielle Treffer wiederholt. Das ist kein vollständiger 32/32-Transport-PASS und bleibt im unabhängigen Review gedrosselt zu wiederholen. Die 112 Aliasvarianten sind ausschließlich ein lokaler Normalisierungstest und kein Live-Suchclaim.
- Nächster Schritt: Nach Push öffentliche API-Regression der 32 gemeldeten Varianten plus die mindestens 100 lokalen Aliasvarianten prüfen; danach unabhängigen Search Reviewer mit PASS/FAIL nach der Acceptance-Matrix beauftragen.
