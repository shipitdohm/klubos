...
### 2026-07-20 — VS-0.7 kanonischer DTB-Aliasvertrag und sicherer OSM-Fallback

- Commit/Diff: Working tree; `api/vereinssuche-normalization.js` führt die kanonische Namensnormalisierung, Aliasvarianten und stabile Identitätsschlüssel ein. `api/vereinssuche.js` fragt nuLiga mit TC/Tennisclub-, Unicode/ASCII-, Bindestrich- und e.V.-Varianten ab, beendet offizielle Mehrdeutigkeit ohne OSM-Ersatz und akzeptiert OSM nur bei kanonisch passendem Namen. `vereinssuche.html` zeigt VS-0.7 und erklärt den sicheren Fallback. `scripts/vereinssuche-alias-contract.mjs` plus `package.json` ergänzen den lokalen Vertragstest.
- Specialist-Status: `READY_FOR_REVIEW` nach Push und öffentlicher Verifikation.
...
- Lokale Tests: `node --check` für beide API-Module erfolgreich; `node scripts/vereinssuche-alias-contract.mjs` erfolgreich mit 18 realen Vereinsankern und 112 Varianten; lokaler Server-Adapter gegen echte nuLiga-/OSM-Quellen: `Tennisclub Großhesselohe` → 1 offizieller DTB-Treffer, `TC Grosshesselohe` → 1 offizieller DTB-Treffer, `TC GH` → 0 `not_found`, `Tennisclub Kirchhörde` → 1 offizieller DTB-Treffer, `Tennisclub Weiden` → 1 offizieller DTB-Treffer, `Tennisclub Bamberg` → 1 offizieller DTB-Treffer, `Tennisclub Neuss` → 0 `ambiguous`, `TC Bredeney` → 1 offizieller DTB-Treffer; direkter Astro-Build erfolgreich (`7 page(s) built`, `Complete!`); `git diff --check` erfolgreich.
