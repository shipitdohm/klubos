...
### 2026-07-20 — VS-0.7.4 kanonischer DTB-Aliasvertrag und sicherer OSM-Fallback

- Commit/Diff: `a7948fd` → `64a9d59` → `26e6164` → `de6d242` plus Follow-up im Working tree; `api/vereinssuche-normalization.js` führt die kanonische Namensnormalisierung, Aliasvarianten, längere Ortsfragmente und stabile Identitätsschlüssel ein. `api/vereinssuche.js` fragt nuLiga mit TC/Tennisclub-, Unicode/ASCII-, Bindestrich- und e.V.-Varianten ab, beendet offizielle Mehrdeutigkeit ohne OSM-Ersatz und akzeptiert OSM nur bei kanonisch passendem Namen. Ein offizieller Namenszusatz wie `TSC Hansa Dortmund` oder `TC Augsburg Siebentisch` wird bei direkter nuLiga-Antwort akzeptiert; generische Stadtanfragen wie `Tennisclub Köln` werden bei einem nur erweiterten Alias-Kandidaten als `ambiguous` behandelt. Eindeutige Ortsfragmente wie `Zündorf` können den vollständigen offiziellen Datensatz auflösen; unbelegte kurze Kürzel wie `TCK` dürfen keinen OSM-Treffer erzeugen; bei `source_unavailable` gibt es keinen OSM-Ersatz. Die strenge Gleichheitsprüfung bleibt ausschließlich beim OSM-Fallback. `vereinssuche.html` zeigt VS-0.7.4 und erklärt den sicheren Fallback. `scripts/vereinssuche-alias-contract.mjs` plus `package.json` ergänzen den lokalen Vertragstest.
- Specialist-Status: `READY_FOR_REVIEW` nach Push und öffentlicher Verifikation.
...
- Inferred: Die gemeldeten Verwechslungen werden dadurch entweder auf den offiziellen DTB-Treffer normalisiert (`Tennisclub Großhesselohe`, `TC Grosshesselohe`, `Tennisclub Kirchhörde`, `Tennisclub Weiden`, `Tennisclub Bamberg`, `TC Augsburg`, `Tennisclub Blau Weiß Zündorf`) oder sicher als `ambiguous`/`not_found` behandelt (`TC GH`, `TCK`, `Tennisclub Neuss`, `Tennisclub Köln`), statt einen ähnlichen Nachbarverein auszugeben. Die VS-0.7.4-Regressionsprobe bestätigt zusätzlich `TSC Hansa` → `TSC Hansa Dortmund`, `TC Augsburg` → `TC Augsburg Siebentisch` und den Zündorf-Vollnamen als offizielle Treffer.
...
- Öffentliche VS-0.7.4-Verifikation nach Push `936a061`: Seite liefert `data-version="VS-0.7.4-official-alias-safe"` und `Live-Quelle VS-0.7.4`. `Tennisclub Blau Weiß Zündorf` und `TC Blau-Weiß Zündorf` liefern jeweils genau 1 offiziellen DTB-/nuLiga-Treffer `TC Blau-Weiss Zündorf`, Köln; `Tennisclub Großhesselohe` und `TC Grosshesselohe` liefern denselben offiziellen Datensatz; `TC Augsburg` liefert `TC Augsburg Siebentisch` offiziell; `TC GH` → 0 `not_found`; `TCK` → 0 `ambiguous`; `Tennisclub Köln` → 0 `ambiguous`. Quellenfelder und höchstens ein Ergebnis bleiben vorhanden. Der unabhängige VS-0.7.4-Review ist noch auszuführen.
